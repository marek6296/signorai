...
export default function BotLayoutPage(){
  ...
  // ── Load from Supabase ───────────────────────────────────────────────────────
  useEffect(()=>{
    supabase.from("site_settings").select("value").eq("key","bot_workflow").single().then(({data})=>{
      if(data?.value){try{setWorkflow(JSON.parse(data.value as string));}catch{}}
    });
  },[]);
  ...
  const save=useCallback(async()=>{
    await supabase.from("site_settings").upsert({key:"bot_workflow",value:JSON.stringify(workflow)},{onConflict:"key"});
  ...
  // ── Wrapper mouse down — starts PAN on background ───────────────────────────
  function wrapMouseDown(e:React.MouseEvent<HTMLDivElement>){
    // Modules stopPropagation their mousedown, so this only fires on background
  ...
  function wrapMouseMove(e:React.MouseEvent<HTMLDivElement>){
    ...
    if(dragRef.current){
      ...
      setWorkflow(w=>({...w,modules:w.modules.map(m=>m.id===dragRef.current!.id?{...m,x:nx,y:ny}:m)}));
      ...
      setView(v=>({...v,panX:panRef.current!.startPx+dx,panY:panRef.current!.startPy+dy}));
    }
  }
  ...
  function portIn(e:React.MouseEvent,modId:string){
    ...
    if(!connections.some(c=>c.fromId===connecting&&c.toId===modId)){
      setWorkflow(w=>({...w,connections:[...w.connections,{id:cuid(),fromId:connecting,toId:modId}]}));
    }
    setConnecting(null);
  }
  ...
  function fitView(){
    ...
    const maxX=Math.max(...modules.map(m=>m.x+MW))+40;
    const maxY=Math.max(...modules.map(m=>m.y+MH))+40;
  ...
  return (
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"#080808",color:"#fff",overflow:"hidden"}}>
      ...
      {/* ── TOP BAR ── */}
      <div style={{height:50,flexShrink:0,display:"flex",alignItems:"center",gap:10,padding:"0 16px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:"rgba(8,8,8,0.98)",backdropFilter:"blur(12px)"}}>
        <span style={{fontSize:17}}>🤖</span>
        <span style={{fontSize:13,fontWeight:800,letterSpacing:"-0.01em"}}>Bot Layout</span>
        <span style={{fontSize:9,color:"rgba(255,255,255,0.2)",fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",marginLeft:2}}>Workflow Builder</span>
        <div style={{width:1,height:18,background:"rgba(255,255,255,0.07)",margin:"0 6px"}}/>
        <input value={name} onChange={e=>setWorkflow(w=>({...w,name:e.target.value}))}
          style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:7,padding:"4px 10px",color:"#fff",fontSize:12,fontWeight:600,outline:"none",width:200}}/>
        <div style={{flex:1}}/>

        {/* Zoom controls */}
        <div style={{display:"flex",alignItems:"center",gap:4,background:"rgba(255,255,255,0.04)",borderRadius:8,padding:"3px 6px",border:"1px solid rgba(255,255,255,0.07)"}}>
        ...
        <button onClick={fitView} style={{padding:"5px 10px",borderRadius:7,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",color:"rgba(255,255,255,0.55)",fontSize:11,cursor:"pointer",fontWeight:600}}>↔ Fit</button>

        {connecting&&(
          <button onClick={()=>setConnecting(null)} style={{padding:"5px 12px",borderRadius:7,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.25)",color:"#ef4444",fontSize:11,fontWeight:700,cursor:"pointer"}}>✕ Zrušiť</button>
        )}
        <button onClick={save} style={{padding:"6px 14px",borderRadius:8,background:saved?"rgba(34,197,94,0.1)":"rgba(255,255,255,0.06)",border:`1px solid ${saved?"rgba(34,197,94,0.3)":"rgba(255,255,255,0.1)"}`,color:saved?"#22c55e":"rgba(255,255,255,0.65)",fontSize:12,fontWeight:700,cursor:"pointer",transition:"all 0.2s"}}>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        {/* ── PALETTE ── */}
        <div style={{width:190,flexShrink:0,background:"#0d0d0d",borderRight:"1px solid rgba(255,255,255,0.06)",overflowY:"auto",padding:"10px 8px"}}>
          ...
          {PALETTE.map(type=>{
            ...
            return (
              <button key={type} onClick={()=>addModule(type)}
                ...
                onMouseEnter={e=>{(e.currentTarget as HTMLButtonElement).style.background=d.bg;(e.currentTarget as HTMLButtonElement).style.borderColor=d.border;}}
          ...
          <p style={{fontSize:9,color:"rgba(255,255,255,0.12)",padding:"6px 4px 0",lineHeight:1.4}}>Klikni pre pridanie na plátno</p>
        </div>
